...
\begin{thm}[15]
	...
	\begin{proof}
		...
		\begin{flalign*}
			\overline{\left(\sum_{n=-N}^{N}{a_{n}z^{n}}\right)} & = \sum_{n=-N}^{N}{\overline{a_n z^{n}}}      = \sum_{n=-N}^{N}{\overline{a_n}z^{-n}} \\
...
\begin{proof}[\ref{thm:8.16}]
	\begin{description}
		\item[$L^2$ convergence:]\hfill \\
		      Let $\epsilon>0$. \\
		      Choose a continuous $h$ (Problem~\nameref{prob:6.12}) s.t. $\|f-h\|_2 < \frac{\epsilon}{3}$.\\
		      Then \[
